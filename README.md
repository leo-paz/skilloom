...
The Library combines global and project skills across local checkouts and published remote observations. The list is always the active control while browsing. Its read-only preview summarizes the selected skill by source, ownership and machine. Enter or `i` opens the same full-width details view; repeated installations with the same facts are grouped. Details include checkout paths, full agent coverage, invocation declarations, usage evidence, and observation timestamps; use ↑/↓ to scroll and Esc to return. Details use a flat layout with blue machine headings and one lavender heading per repository or global installation. Skill sources remain separately labelled. Uniform invocation declarations share a line, observation times appear once per machine, and fleet evidence coverage follows the installations. Different sources and ownership remain distinct between independent clones. Unknown sources remain visible.

Rows show **Invoke** (declared invocation mode) and **Evidence** (verified installation reads), alongside ownership and machine count when space permits. Invoke is Manual, Auto, Both, Disabled, Mixed, Partial, or `?` when unknown. Mixed means known declarations disagree; Partial means some occurrences have no known declaration. Modes describe supported declarations, not runtime settings or permissions; open details for the per-harness declaration evidence. Claude frontmatter flags, Codex `agents/openai.yaml` policy, and Pi declarations are interpreted separately.

**Evidence** lists OpenAI, Claude, and Pi only when a supported trace records a successful read of a verified installed `SKILL.md` path. Compact terminals use OAI / Cl / Pi. Reads prove that the file was loaded, not that its instructions were followed or that the current file contents match its historical contents. Matching uses machine identity plus canonical installation-path hashes; same-name copies in other checkouts are kept separate. Name-only Claude Skill invocations appear separately in details because their installation cannot be established.

**Unscanned** means no compatible evidence snapshot was collected. **Partial** means scan limits were reached or selected machines have no evidence snapshot. **No match** means the supported scan finished without matching evidence; it never means “never used.” Details show selected-machine coverage, collection times, read counts and last seen. All-machines coverage includes registered machines even when they have no published installations. Availability and declared invocation mode are independent of evidence.

Each machine collects its own logs. Run `skilloom observe --publish` on each machine using a version with this collector, then refresh the shared observations. Git transports summaries; it does not discover online machines or run remote collectors. Raw traces and absolute skill paths stay local. Published summaries contain machine identity, canonical-path hashes, harness, counts, last-seen times and scan coverage. Legacy usage summaries are treated as unscanned until recollected.
