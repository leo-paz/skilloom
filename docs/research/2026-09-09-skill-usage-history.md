...
## Bounded runtime and measured overhead

Historical collection now uses a two-minute budget per CLI invocation or dashboard refresh. It finishes saving the current bounded batch, preserves its cursor, and reports a paused state if history remains. Automatic recent-event refreshes cannot reset that budget. An explicit refresh or another CLI invocation allows another bounded run. `--max-seconds` accepts 1–180 seconds. This is a cooperative cutoff with checkpoint-write overhead, not a forced process termination.

The original MacBook backfill accumulated 71,851 file cursors and a 31.37 MB JSON checkpoint. Every small pass repeated cache decoding, cursor validation, coverage timestamp sorting and checkpoint serialization. Profiling that completed checkpoint measured approximately 0.5 seconds per pass in timestamp sorting alone. The fix uses linear oldest/newest selection, metadata-validated checkpoint reuse in the same process, bounded directory-entry pages, set membership for path identities, and 192-file historical batches. Recent scans retain their smaller budget. Structured event validation and successful-result pairing are preserved.

A disposable 20,000-cursor benchmark measured median repeated-pass time falling from 115 ms to 37 ms. This isolates cache processing and does not predict full-archive runtime. `python3 scripts/usage-budget-acceptance.py` tests the packaged CLI against 10,000 disposable trace files: with a one-second budget, two successive invocations paused near the limit and increased retained counts without resetting progress. The two-minute cutoff and dashboard tail-only behavior after expiry are also covered with a controlled monotonic clock.

## Session usage

Skill details now report distinct recorded sessions for the selected installation and agent. Recent sessions has one row per machine, harness and verified native session, merging repeated reads/loads and repeated inventory occurrences. Installed path identity remains the attribution boundary; name-only invocations do not establish use of every matching installation. Scan coverage no longer shows file counts in this user-facing view.

Grouping uses all retained normalized events before the separate 1,000-event public history cap. Summaries remain bounded by the 10,000-event retention window and are labelled as observed evidence, with explicit truncation. Missing native IDs are never replaced by filenames, timestamps or event IDs. Legacy ambiguous Codex evidence retains its read/load count but is excluded from session counts until verified.

Codex session metadata distinguishes a thread ID from the root session ID. The root groups subagent work while the original thread still participates in existing event identities for deduplication. Supported inherited fork records are suppressed, and conflicting verified attributions become unknown instead of choosing the last scanned file. Bounded header reads can upgrade legacy root-session evidence without rereading entire trace bodies or resetting file offsets. Claude session IDs group nested agent work; Pi uses its session header identity, matching the extension runtime's session ID. Raw session identifiers and transcript bodies remain private.

At the Pi fork timestamp boundary, equal-millisecond records are treated as ambiguous and excluded from new attribution. Existing retained records are marked session-unknown only when a cached cursor positively identifies the matching fork and boundary; event IDs and raw event counts are preserved.
