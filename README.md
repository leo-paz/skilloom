...
The Library combines global and project skills across local checkouts and published remote observations. The list is always the active control while browsing. Its read-only preview summarizes the selected skill by source, ownership and machine. Enter or `i` opens the same full-width details view; repeated installations with the same facts are grouped. Details lead with the last recorded use, then show installation locations, source, invocation and recent sessions. Press `e` for diagnostics such as reader errors, full agent coverage and observation timestamps; press `e` again for the concise view. Use ↑/↓ to scroll and Esc to return. Details use aligned labels and values, with installation facts and agent behavior side by side in wide terminals and stacked in narrow ones. Machine and repository headings identify each installation without nesting. Session usage appears in a machine-by-agent table, long checkout paths wrap at directory boundaries, and observation times appear once per machine. Different sources and ownership remain distinct between independent clones. Unknown sources remain visible.
...
Press **b** to switch between the default **Flat** list and **By source**. Source groups use the skill's recorded origin, such as `anthropics / skills`, not the project where it is installed. Enter on a source heading expands or collapses it; Enter or `i` on a skill opens details. Search reveals matching skills inside groups even when a group was collapsed, and machine, scope, and ownership filters apply before grouping. Same-name skills from different sources stay in separate groups. Unverified origins appear last under **Source unknown**.

Rows show the skill name and **Invocation**, followed by ownership and machine count when space permits. Invocation shows **Automatic**, **Manual**, or **Disabled**. **Automatic** includes skills that support both automatic and manual invocation. **Mixed** means copies have different invocation modes after grouping automatic-capable skills together; **Unknown** means declarations are unknown or incomplete for installed copies. **—** means the skill is not installed on the selected machines. Missing requirements do not obscure the modes of installed copies. The column respects the machine filter. Exact declarations and read evidence stay in the full details view opened with Enter or `i`.

Invocation describes supported declarations, not runtime settings or permissions. Details identify the agents whose declarations were read and preserve differences between them. Claude and OpenClaw frontmatter flags, Codex `agents/openai.yaml` policy, and Pi declarations are interpreted separately. Details distinguish an unsupported agent reader, an unreadable declaration, and a snapshot that has not collected declarations.
...
The session table compares machines across OpenAI, Claude and Pi. It distinguishes verified counts, zero recorded matches, unknown identities, missing observations and absent installations. No recorded evidence never means “never used.” Details show how many distinct recorded sessions used the selected installation, with last-use time. Repeated reads and loads in one session count once. **Recent sessions** lists the latest five sessions, or 20 with diagnostics open, with UTC last-use time, agent, an abbreviated opaque session ID and machine. The diagnostics view explains gaps such as omitted earlier history, unknown session identities or an explicit pause. The normal view keeps a brief reminder when history is incomplete. Repeated generic scan-status tables are omitted. Name-only invocations are labelled separately. Older collectors without session summaries show “Sessions not collected.” Evidence without a trustworthy session identity is shown as unknown and does not increase session counts. Coverage includes registered machines even when they have no published installations. Availability and declared invocation mode are independent of evidence.

Session counts use a persistent index of normalized evidence, independent of the small recent-history display. Each skill counts once per native session on a machine and harness, even when read repeatedly or from several matching installations. The scanner streams transcript records and keeps normalized evidence on disk; it does not load entire transcripts into memory. Codex subagent threads group under the root session when its metadata identifies it. Claude Code uses its session ID, including subagent work within that session. Pi uses its native session ID and excludes inherited fork history. Name-only invocations remain separate from verified installation usage. Legacy ambiguous Codex identities do not become session counts until original metadata verifies them. Completing a scan means processing the retained supported logs, not recovering deleted logs or proving invocations that a harness never records reliably. Counts update after a completed historical pass; a paused pass preserves the last completed counts.

Each machine collects its own logs. Codex reads `sessions` and `archived_sessions` under `CODEX_HOME` (default `~/.codex`); Claude reads `projects` under `CLAUDE_CONFIG_DIR` (default `~/.claude`); Pi reads `sessions` under `PI_CODING_AGENT_DIR` (default `~/.pi/agent`). Changing a configured log root starts a new traversal, while the durable index deduplicates retained session identities. `skilloom usage install` adds a Claude Code PostToolUse hook and a Pi extension while preserving existing configuration and writing backups. Reload Pi or start a new session after installation. `skilloom usage status` reports configuration; `skilloom usage uninstall` removes only Skilloom's collectors. Codex is observed through structured rollout metadata and successful tool results without changing its configuration.

`skilloom usage backfill` processes discovered historical logs to completion by default, checkpointing between bounded batches. Ctrl-C pauses safely; run it again to resume. Use `--max-seconds 180` only if you want an optional deadline, `--once` for one batch, or `--restart` to traverse directories again. `skilloom usage refresh` collects recent evidence. The TUI continues historical batches while open, then switches to recent-event refreshes. Selecting rows never starts filesystem work. Complete traversal means the discovered files were processed; unsupported, unreadable or ambiguous records remain explicit evidence gaps.

The append-only journal lives in the private Skilloom `usage` directory, alongside a private installation manifest and collection state. Shared event IDs deduplicate hooks and traces. Historical scanning can finish with partial evidence because malformed records, ambiguous fork boundaries, missing files and unsupported events remain visible limitations. Large tool-result bodies are streamed; unusually large structured invocation fields that exceed the projection limits remain explicit gaps. Machine-readable diagnostics retain scanned-file and pending-result counts; the TUI presents session usage and explains evidence gaps.
...
Saved inventory renders immediately. Older snapshots missing metadata or history, or containing declarations from an older reader, are enriched in the background using bounded local file reads, without project discovery or Git access; browsing remains available. Snapshots containing current declarations and history need no startup scan. Remote declarations refresh only when their originating machine publishes a new observation. Explicit refresh/observe scans declarations and updates a bounded incremental usage cache outside the render path. Trace content remains local; published observations contain only invocation metadata and derived usage evidence. Remote rows keep the publication timestamp and remain snapshots.
