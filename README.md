...
The Library combines global and project skills across local checkouts and published remote observations. The list is always the active control while browsing. Its read-only preview summarizes the selected skill by source, ownership and machine. Enter or `i` opens the same full-width details view; repeated installations with the same facts are grouped. Details include checkout paths, full agent coverage, invocation declarations, usage evidence, and observation timestamps; use ↑/↓ to scroll and Esc to return. Details group installations under blue machine headings, with project repositories and skill sources labelled separately in lavender. Indented paths and metadata stay beneath their installation; fleet evidence coverage follows the installations. Different sources and ownership remain distinct between independent clones. Unknown sources remain visible.
