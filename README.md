...
The coverage table distinguishes unscanned machines from partial and complete supported scans. No recorded evidence never means “never used.” Details show how many distinct recorded sessions used the selected installation, with last-use time. Repeated reads and loads in one session count once. **Recent sessions** lists the latest 20 sessions, with UTC last-use time, agent, an abbreviated opaque session ID and machine. Scan status remains separate and no longer presents trace-file counts as usage. Name-only invocations are labelled separately. Older collectors without session summaries show “Sessions not collected.” Evidence without a trustworthy session identity is shown as unknown and does not increase session counts. Coverage includes registered machines even when they have no published installations. Availability and declared invocation mode are independent of evidence.
