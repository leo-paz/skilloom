...
Rows show the skill name and **Invocation**, followed by ownership and machine count when space permits. Invocation shows **Auto**, **Manual**, **Both**, or **Disabled**. **Mixed** means known declarations disagree; **—** means declarations are unknown or incomplete for the selected occurrences. The column respects the machine filter. Read evidence stays in the full details view opened with Enter or `i`.
